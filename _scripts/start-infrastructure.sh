...
start_service "Registry" 8000 "SERVICES/registry" \
    "uvicorn app.main:app --host 0.0.0.0 --port 8000" \
    "/health"

# 2. Orchestrator (Port 8001) - Task Routing
start_service "Orchestrator" 8001 "SERVICES/orchestrator" \
    "uvicorn app.main:app --host 0.0.0.0 --port 8001" \
    "/orchestrator/health"

# 3. SPEC Verifier (Port 8002) - Service Validation
if [ -d "SERVICES/spec-verifier" ]; then
    start_service "SPEC-Verifier" 8002 "SERVICES/spec-verifier" \
...
if [ -d "SERVICES/unified-chat" ]; then
    start_service "Unified-Chat" 8100 "SERVICES/unified-chat" \
        "python3 main_secure.py" \
        "/api/health"
fi

# 5. FPAI Hub (Port 8010) - Service Hub
if [ -d "SERVICES/fpai-hub" ]; then
    start_service "FPAI-Hub" 8010 "SERVICES/fpai-hub" \
...
echo "  - Connect sessions: cd SERVICES/unified-chat && python3 connect_session.py"
