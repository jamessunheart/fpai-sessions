...
```
fpai/
├── agents/services/                    # All deployed services (droplets)
...
→ `agents/services/{service}/README.md`

---

## 📋 Directory Responsibilities

### `/agents/services/` - All Services
...
1. Move `i-match` from `/opt/fpai/i-match/` to `/opt/fpai/agents/services/i-match/`
2. Move `i-proactive` from `/opt/fpai/i-proactive/` to `/opt/fpai/agents/services/i-proactive/`
3. Create missing directories (`docs/`, `infra/`)

### Phase 2: Local Reorganization
1. Move all `.md` files from root to `docs/`
2. Rename the previous service directory to `agents/services/`
...
1. Always placing new services in `agents/services/`
