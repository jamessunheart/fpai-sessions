"""champion-bot — generic Telegram bot for First Cohort Champions.

This is the same architecture as sapphire-bot, but driven entirely by env
vars. One Champion = one systemd instance = one env file. The system
prompt is constructed at runtime from the Champion's role summary, voice
register, and audience description (sourced from `core/CHAMPIONS/<slug>.yaml`
at deploy time).

Memory model (SQLite at /var/lib/champion-bot/<slug>/db.sqlite):
  - messages       — full conversation log
  - business_facts — facts the Champion /teach-es the bot
  - leads          — pipeline (name, contact, status, notes)

Owner-gated: first /start with no owner set auto-claims ownership.
"""
...
# ─── Champion config from env ─────────────────────────────────────────────
CHAMPION_SLUG = os.environ.get("CHAMPION_SLUG", "").strip()
if not CHAMPION_SLUG:
    print("CHAMPION_SLUG not set — exiting", file=sys.stderr)
    sys.exit(1)

log = logging.getLogger(f"champion-bot.{CHAMPION_SLUG}")
...
CHAMPION_NAME = os.environ.get("CHAMPION_NAME", CHAMPION_SLUG.title())
CHAMPION_SHORT_NAME = os.environ.get("CHAMPION_SHORT_NAME", CHAMPION_NAME.split()[0])
BOT_PERSONA_NAME = os.environ.get("BOT_PERSONA_NAME", f"{CHAMPION_SHORT_NAME}'s Assistant")
BOT_USERNAME = os.environ.get("BOT_USERNAME", "")

CHAMPION_ROLE_SUMMARY = os.environ.get("CHAMPION_ROLE_SUMMARY", "").strip()
CHAMPION_VOICE_REGISTER = os.environ.get("CHAMPION_VOICE_REGISTER", "warm, direct, brief, real").strip()
CHAMPION_AUDIENCE = os.environ.get("CHAMPION_AUDIENCE", "").strip()

DATA_DIR = Path(os.environ.get("DATA_DIR", f"/var/lib/champion-bot/{CHAMPION_SLUG}"))
...
SYSTEM_PROMPT_TEMPLATE = """You are {persona} — {champion_short}'s personal AI assistant on Telegram.

WHO {champion_short_upper} IS:
{role_summary}

YOUR DUAL ROLE:
You serve TWO different humans differently:
  1. {champion_short} herself (the OWNER) — she will appear with explicit owner context. With her you are operational: drafting, organizing, remembering, reflecting. Match her voice; be direct.
  2. Prospects and leads (everyone else) — with them you are not a customer service rep. You are the gatekeeper who screens before pricing. You hold the frame of {champion_short}'s tier.

POSTURE WITH PROSPECTS — read this carefully:
Top-tier practitioners do not chase. Neither do you on {champion_short}'s behalf. When a stranger reaches out about working with {champion_short}, your default is NOT to immediately give them pricing or sell. Your default is to ASK FIRST.
...
Ask 3-5 qualifying questions across the arc of the conversation BEFORE quoting investment levels:
  ...
  - Whether they are clear they want 1:1 work or something lighter
  - Whether they are in a clinical/crisis situation (if yes, gently redirect)

ONLY AFTER you have a sense of their fit do you surface offerings. Route based on what you've heard.

PRICING — handle with confidence, never apology:
When you do quote pricing, state it cleanly. Never soften, never preempt. Do not add "I know that's a lot" — those phrases erode authority. If they ask about payment plans, then mention them.

WILLING TO SAY NO:
A top-tier bot does not place every lead. Some prospects are not the right fit, and saying so respectfully is part of holding the frame.

LANGUAGE REGISTER:
{voice_register}
You match this. Avoid corporate language ("I'd be happy to help!"). Avoid hype ("amazing!"). Avoid emoji except in moments of clear warmth.

WHO {champion_short_upper} SERVES:
{audience}

THE BUSINESS FACTS {champion_short} has taught you below are authoritative. If a prospect asks something not covered there, say you'll check with {champion_short} directly rather than inventing.

SUGGEST COMMANDS TO {champion_short_upper} (only when you're talking to HER, not prospects):
  - `/lead Name | contact | notes` — log a new prospect
  - `/status <id> <new|qualified|replied|booked|lost>` — pipeline movement
  - `/teach <fact>` — when something durable is learned

WHAT YOU NEVER DO:
- Never invent facts about {champion_short}'s offerings, schedule, or training. If unknown, say so.
...
CURRENT CHAT: This is {champion_short} herself (the OWNER). Be operational. Help her draft, organize, remember, and reflect. Suggest /lead, /status, /teach commands when relevant. Use her time well — short replies unless she asks for depth. You are her co-pilot, not her gatekeeper, in this chat."""


PROSPECT_ADDENDUM = """

CURRENT CHAT: This is a PROSPECT or LEAD — not {champion_short}. Hold the gatekeeper frame described above. Default to curiosity and qualifying questions. Do not surface pricing in the first message. Do not use any slash commands in your reply (those are for {champion_short}, not prospects). Keep responses to 2-3 short paragraphs. End with a single open question that moves the conversation toward fit assessment."""


def build_system_prompt(is_owner_chat: bool) -> str:
    base = SYSTEM_PROMPT_TEMPLATE.format(
        persona=BOT_PERSONA_NAME,
        champion_short=CHAMPION_SHORT_NAME,
        champion_short_upper=CHAMPION_SHORT_NAME.upper(),
        role_summary=CHAMPION_ROLE_SUMMARY or f"{CHAMPION_NAME} runs a high-tier practice. Their offerings, lineage, and voice will be taught to you over time via /teach.",
        voice_register=CHAMPION_VOICE_REGISTER,
        audience=CHAMPION_AUDIENCE or "the people they are best positioned to serve",
    )
    facts = list_facts()
    parts = [base]
    if facts:
        parts.append("\n\nBUSINESS FACTS THE CHAMPION HAS TAUGHT YOU:")
        for f in facts:
            parts.append(f"  [{f['id']}] {f['fact']}")
    else:
        parts.append("\n\nBUSINESS FACTS: (none yet — when they share durable facts about their work, suggest /teach so you remember next session.)")

    addendum = (OWNER_ADDENDUM if is_owner_chat else PROSPECT_ADDENDUM).format(
        champion_short=CHAMPION_SHORT_NAME,
    )
...
def welcome_owner() -> str:
    return f"""💎 <b>{BOT_PERSONA_NAME} is online.</b>
...
WELCOME_PROSPECT_FALLBACK = """Thank you for reaching out. I'll see your message and respond personally."""
...
async def cmd_start(client, chat_id: int, args: str) -> None:
    if not OWNER_TG_ID:
        ...
        await tg_send(client, chat_id,
            ...
            f"is now the owner of @{BOT_USERNAME}.\n\n" + welcome_owner())
        return
    if is_owner(chat_id):
        await tg_send(client, chat_id, welcome_owner())
    else:
        # Let Claude greet — proper screening starts here, not a hardcoded line
        contact = f"tg:{chat_id}"
        if not lead_exists_by_contact(contact):
            add_lead(name="", contact=contact, notes="auto-logged from /start")
        opener = f"Hi — I'm {BOT_PERSONA_NAME}, {CHAMPION_SHORT_NAME}'s assistant. What's calling you to reach out today?"
        save_message(chat_id, "assistant", opener)
        await tg_send(client, chat_id, opener)
...
async def cmd_help(client, chat_id: int, args: str) -> None:
    if is_owner(chat_id):
        ...
        await tg_send(client, chat_id, WELCOME_PROSPECT_FALLBACK)


async def cmd_teach(client, chat_id: int, args: str) -> None:
    if not is_owner(chat_id):
        await tg_send(client, chat_id, f"Only {CHAMPION_SHORT_NAME} can teach me.")
        return
    fact = (args or "").strip()
    if not fact:
        await tg_send(client, chat_id,
            "Tell me something durable about your business.\n\n"
            "Example: <code>/teach my services are X, Y, Z. Pricing starts at $...</code>")
        return
    fid = add_fact(fact)
    await tg_send(client, chat_id,
...
async def cmd_memory(client, chat_id: int, args: str) -> None:
    if not is_owner(chat_id):
        await tg_send(client, chat_id, f"Only {CHAMPION_SHORT_NAME} can see what I remember.")
        return
    facts = list_facts()
    if not facts:
        await tg_send(client, chat_id,
...
async def cmd_forget(client, chat_id: int, args: str) -> None:
    if not is_owner(chat_id):
        await tg_send(client, chat_id, f"Only {CHAMPION_SHORT_NAME} can edit my memory.")
        return
    try:
        fid = int((args or "").strip())
    except ValueError:
        await tg_send(client, chat_id, "Usage: <code>/forget &lt;id&gt;</code>")
        return
    if delete_fact(fid):
...
async def cmd_lead(client, chat_id: int, args: str) -> None:
    if not is_owner(chat_id):
        await tg_send(client, chat_id, f"Only {CHAMPION_SHORT_NAME} can log leads.")
        return
    raw = (args or "").strip()
    if not raw:
        await tg_send(client, chat_id,
...
async def cmd_leads(client, chat_id: int, args: str) -> None:
    if not is_owner(chat_id):
        await tg_send(client, chat_id, f"Only {CHAMPION_SHORT_NAME} can see the pipeline.")
        return
    status = (args or "").strip().lower() or None
    if status and status not in LEAD_STATUSES:
        await tg_send(client, chat_id,
            f"Unknown status. Use one of: {', '.join(LEAD_STATUSES)}.")
        return
...
async def cmd_status(client, chat_id: int, args: str) -> None:
    if not is_owner(chat_id):
        await tg_send(client, chat_id, f"Only {CHAMPION_SHORT_NAME} can update leads.")
        return
...
async def cmd_draft(client, chat_id: int, args: str) -> None:
    if not is_owner(chat_id):
        await tg_send(client, chat_id, f"Only {CHAMPION_SHORT_NAME} can use draft.")
        return
    msg = (args or "").strip()
    if not msg:
        await tg_send(client, chat_id, "Paste the lead's message after the command.")
        return
    prompt = (
        f"A lead just sent {CHAMPION_SHORT_NAME} this message:\n\n\"{msg}\"\n\n"
        f"Draft her reply in her voice. Keep it short. If it's a delicate or pricing question, "
        f"give 2 short variations labeled A and B. Don't add commentary outside the draft itself."
    )
    reply = await chat_with_claude(client, chat_id, prompt)
    if reply:
        await tg_send(client, chat_id, f"✏️ <b>Draft:</b>\n\n{esc(reply)}")
    else:
        await tg_send(client, chat_id, "I can't draft right now — Anthropic key missing or unreachable.")


async def cmd_digest(client, chat_id: int, args: str) -> None:
    if not is_owner(chat_id):
        await tg_send(client, chat_id, f"Only {CHAMPION_SHORT_NAME} can see the digest.")
        return
    rows = list_leads()
    ...
    lines = [
        f"🌅 <b>{BOT_PERSONA_NAME.upper()} DIGEST</b>",
...
async def main():
    db_init()
    log.info(
        "champion-bot[%s] starting; bot=@%s persona=%s model=%s owner=%s",
        CHAMPION_SLUG, BOT_USERNAME, BOT_PERSONA_NAME, ANTHROPIC_MODEL,
        OWNER_TG_ID or "(unclaimed — first /start wins)",
    )
    offset = _load_offset()
