...
mkdir -p "$COORD_DIR/heartbeats"
cat > "$COORD_DIR/heartbeats/${TIMESTAMP}-${SESSION_ID}.json" <<EOF
{
...
with open('${COORD_DIR}/sessions/${SESSION_ID}.json', 'r') as f:
    data = json.load(f)
...
with open('${COORD_DIR}/sessions/${SESSION_ID}.json', 'w') as f:
    json.dump(data, f, indent=2)
"
fi

# Cleanup old heartbeats (older than 24 hours)
find "$COORD_DIR/heartbeats" -name "*.json" -mtime +1 -delete 2>/dev/null || true

# Auto-update CURRENT_STATE.md timestamp to keep state fresh
CURRENT_STATE_FILE="$COORD_DIR/sessions/CURRENT_STATE.md"
...
BROADCAST_COUNT=$(ls -1 "$COORD_DIR/messages/broadcast"/*.json 2>/dev/null | wc -l | tr -d ' ')
DIRECT_COUNT=0
if [ -d "$COORD_DIR/messages/direct/${SESSION_ID}" ]; then
    DIRECT_COUNT=$(ls -1 "$COORD_DIR/messages/direct/${SESSION_ID}"/*.json 2>/dev/null | wc -l | tr -d ' ')
fi
...
HEARTBEAT_COUNT=$(python3 -c "import json; print(json.load(open('${COORD_DIR}/sessions/${SESSION_ID}.json')).get('heartbeat_count', 0))" 2>/dev/null || echo "0")
if [ $((HEARTBEAT_COUNT % 10)) -eq 0 ]; then
    "$COORD_DIR/scripts/session-capture-knowledge.sh" > /dev/null 2>&1 || true
fi

# Update status board
"$COORD_DIR/scripts/update-status-board.sh" 2>/dev/null || true
...
if [ $((HEARTBEAT_COUNT % 5)) -eq 0 ]; then
    ...
    RECENT_CRITICAL=$(find "$COORD_DIR/messages/broadcast" -type f -name "*.json" -mmin -120 2>/dev/null | xargs grep -l "CREDENTIAL VAULT\|CRITICAL\|URGENT" 2>/dev/null | head -5)
