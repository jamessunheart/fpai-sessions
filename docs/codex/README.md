...
4. The spec you're building: `docs/codex/specs/SPEC_<name>.md`.
...
1. **Service Registry / World Map** — next approved direction from `tools/scout/SCOUT_REPORT.md`; write/approve a spec before building. Goal: read-only inventory + status classification.
2. **`SPEC_daily-realtime`** — keep HOME/NEXT/proof progress current from live sources. Branch TBD by spec.
3. **`SPEC_multimodel-debate-harness`** — real Claude+GPT+Gemini debates. Branch `feat/multimodel-debate`.
4. **`SPEC_financial-consolidation-hub`** + **`SPEC_communication-hub`** — the hubs (build multi-tenant-ready per the product direction).
5. **`SPEC_oss-bulk-lane`** — defer until a hub needs it.
...
> Read `AGENTS.md`, then `docs/codex/README.md`, then `docs/codex/HANDOFF.md`, then `docs/codex/BRAIN_SYNC.md`, then the target spec. Work ONLY on the branch named in the spec. Touch only the files-allowed; never the files-forbidden. Build to the Definition of Done, run the tests, then update the 📥 lane in `docs/codex/HANDOFF.md` and give me: files changed · summary · tests · risks · rollback. Do not merge — show me the diff first.
