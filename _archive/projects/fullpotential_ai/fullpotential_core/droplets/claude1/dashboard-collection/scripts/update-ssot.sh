...
cat > "$SSOT_FILE" << EOF
{
  ...
  "claude_sessions": $(cat "$COORDINATION_DIR/claude_sessions.json" 2>/dev/null || echo '{}'),
  "services": $(cat "$COORDINATION_DIR/services_status.json" 2>/dev/null || echo '{"total": 0, "services": []}')
}
EOF
