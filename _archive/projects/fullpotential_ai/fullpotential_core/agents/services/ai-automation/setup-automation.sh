...
0 6 * * * cd /Users/jamessunheart/Development/agents/services/ai-automation && /usr/local/bin/python3 orchestrator.py >> logs/orchestrator_$(date +\%Y\%m\%d).log 2>&1

# ============================================================
# PROSPECT IMPORT - Find new prospects daily
# ============================================================
# Runs at 7am every day (after orchestrator identifies any issues)
0 7 * * * cd /Users/jamessunheart/Development/agents/services/ai-automation && /usr/local/bin/python3 -c "from marketing_engine.integrations.apollo import ApolloClient; from marketing_engine.api import create_campaign; apollo = ApolloClient(); results = apollo.search_people(job_titles=['CEO', 'Founder'], company_size='10-100', per_page=100); print(f'Found {len(results.get(\"people\", []))} prospects')" >> logs/daily_prospect_import.log 2>&1
...
0 2 * * * cd /Users/jamessunheart/Development/agents/services/ai-automation && tar -czf backups/campaigns_$(date +\%Y\%m\%d).tar.gz data/ && find backups/ -name "campaigns_*.tar.gz" -mtime +30 -delete

# ============================================================
# CLEANUP - Remove old logs to prevent disk full
# ============================================================
# Runs at 3am every day
0 3 * * * find /Users/jamessunheart/Development/agents/services/ai-automation/logs -name "*.log" -mtime +30 -delete

# ============================================================
# WEEKLY REPORT - Email summary every Monday
# ============================================================
# Runs at 8am every Monday
0 8 * * 1 cd /Users/jamessunheart/Development/agents/services/ai-automation && /usr/local/bin/python3 orchestrator.py weekly-report >> logs/weekly_reports.log 2>&1

# ============================================================
# MONTHLY ANALYTICS - Deep analysis first of month
# ============================================================
# Runs at 9am on the 1st of each month
0 9 1 * * cd /Users/jamessunheart/Development/agents/services/ai-automation && /usr/local/bin/python3 orchestrator.py monthly-analysis >> logs/monthly_analytics.log 2>&1
