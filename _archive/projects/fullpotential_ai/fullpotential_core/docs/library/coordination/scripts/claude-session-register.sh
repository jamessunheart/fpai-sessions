...
# Auto-cleanup stale sessions before registration
...
# Check if number already taken by an ACTIVE session
if echo "$CURRENT" | python3 -c "import sys, json; data=json.load(sys.stdin); active=[k for k,v in data.items() if v.get('status') == 'active']; sys.exit(0 if '$NUMBER' in active else 1)" 2>/dev/null; then
    echo "❌ ERROR: Number $NUMBER is already taken by an ACTIVE session!"
    echo ""
    echo "Active sessions:"
    cat "$SESSIONS_FILE" | python3 -c "import sys, json; data=json.load(sys.stdin); active={k:v for k,v in data.items() if v.get('status')=='active'}; [print(f\"  #{k}: {v['role']}\") for k,v in sorted(active.items())]"
    echo ""
    echo "Inactive sessions (available to reclaim):"
    cat "$SESSIONS_FILE" | python3 -c "import sys, json; data=json.load(sys.stdin); inactive={k:v for k,v in data.items() if v.get('status')!='active'}; [print(f\"  #{k}: {v['role']}\") for k,v in sorted(inactive.items())]" || echo "  None"
    exit 1
fi
