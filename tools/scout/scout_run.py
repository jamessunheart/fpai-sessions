...
def run_if_due(
    *,
    ...
    rendered = render_outputs(payload, run_date)
    targets = {
...
def render_outputs(payload: dict[str, Any], run_date: str) -> dict[str, str]:
    return {
        "news": render_news(payload["news"], run_date),
        "growth": render_growth(payload["growth"], run_date),
    }


def render_news(items: list[dict[str, Any]], run_date: str) -> str:
    lines = [
        "---",
...
def render_growth(items: list[dict[str, Any]], run_date: str) -> str:
    lines = [
        "---",
        f"status: live (scout pipe - last run {run_date})",
        f"last_run: {run_date}",
